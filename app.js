...
function renderReports(){
  ...
  $('#rToday').textContent=fmtM(today);$('#rWeek').textContent=fmtM(week);$('#rStreak').textContent=state.streak.count||0;$('#rDue').textContent=dueRevisions().length;
  $('#rTotalStudy').textContent=fmtM(totalMins);$('#rFormalProof').textContent=`${formalMastered}/${FORMAL_LESSONS.length}`;$('#rProjectProof').textContent=`${projectMastered}/${projectLessons.length}`;$('#rGateProof').textContent=`${gp}/${DATA.gates.length}`;
  const evidenceN=state.evidence.length,complete=formalMastered===FORMAL_LESSONS.length&&projectMastered===projectLessons.length&&gp===DATA.gates.length;
  $('#trainingProofText').textContent=complete?`Core course proof complete in Mentor: ${fmtM(totalMins)} focused study recorded, ${formalMastered} formal lessons, ${projectMastered} project phases and ${gp} protected Gates. Keep your runnable project/evidence artifacts for interviews.`:`Your proof so far: ${fmtM(totalMins)} focused study, ${formalMastered}/${FORMAL_LESSONS.length} formal lessons, ${projectMastered}/${projectLessons.length} project phases, ${gp}/${DATA.gates.length} Gates and ${evidenceN} saved evidence items. The goal is independent capability, not a paid-course receipt.`;
  const max=Math.max(1,...Object.values(minsBy));$('#studyBars').innerHTML=weekKeys.map(k=>`<div class="barRow"><span>${k.slice(5)}</span><div class="barTrack"><i style="width:${Math.round(minsBy[k]/max*100)}%"></i></div><em>${fmtM(minsBy[k])}</em></div>`).join('');
...
function init(){activeLessonId=state.currentLessonId;bind();const s=ls(state.currentLessonId);if(!s.started&&state.currentLessonId===LESSONS[0].id){}renderAll();ensureSessionInterval();if('serviceWorker'in navigator)navigator.serviceWorker.register('sw.js?v=20260912-r7',{updateViaCache:'none'}).catch(()=>{});if(!state.setupSeen)setTimeout(openSetup,250)}
